...
const FruitDetails = () => {
  ...
  const {  freshfruits,
    ...
    loading } = useContext(Createcart);
  const [detailrecord, setDetailrecord] = useState([]);
  
  useEffect(() => {
    const userRecord = freshfruits.find(record => record.key === id);
  ...
  return (
    <div className=" pt-20 ">
        ...
        <p className="flex justify-start items-center px-2">
          <Link to="/fresh-fruits" className="px-2 hover:text-[#00C851]">
          Fresh Fruits
          </Link>{" "}
          / <p className="px-2"> {detailrecord?.data?.name}</p>
        </p>
      </header>
          ...
          <div className="lg:grid lg:auto-rows-min lg:grid-cols-12 lg:gap-x-8">
            <div className="lg:col-span-5 lg:col-start-8">
              <div className="flex justify-between">
                <h1 className="text-xl font-medium text-gray-900">{detailrecord?.data?.name}</h1>
                <p className="text-xl font-medium text-gray-900">RS. {detailrecord?.data?.price}</p>
              </div>
              {/* Reviews */}
              <div className="mt-4">
            ...
            <div className="mt-8 lg:col-span-7 lg:col-start-1 lg:row-span-3 lg:row-start-1 lg:mt-0">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 lg:grid-rows-2 lg:gap-6">
             
                  <img
                    key={detailrecord?.data?.name}
                    alt={detailrecord?.data?.image}
                    src={detailrecord?.data?.image}
